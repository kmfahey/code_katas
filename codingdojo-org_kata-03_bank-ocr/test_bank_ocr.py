...
def test_bulk_process_acct_no_text_2():
    numerals_text_str = """ _  _  _  _  _     _  _    
|_||_   |  ||_  _|| | _||_|
 _| _|  |  ||_| _||_| _|  |
...
|_|  | _| _| _|  ||_   | _|
  ...
  ||_|  |  |  ||_ |_   |  |
"""
    output = bulk_process_acct_no_text_2(numerals_text_str)
    assert output[0] == "957763034"
    assert output[1] == "676714269"
    assert output[2] == "724352737 ERR"
    assert output[3] == "611694485 ERR"
    assert output[4] == "595394485"
    assert output[5] == "777167557"
    assert output[6] == "917383134 ERR"
    assert output[7] == "231312571"
    assert output[8] == "574193294"
    assert output[9] == "631278913 ERR"
    assert output[10] == "515649627"
    assert output[11] == "651703364 AMB"
    assert output[12] == "243223227 ERR"
    assert output[13] == "679937275 ERR"
    assert output[14] == "161112214 ERR"
